...
<template>
    ...
    <div class="w-full min-h-screen bg-[#D6E9FC] py-10 px-6 flex flex-col items-center">
        <div class="not-prose max-w-4xl w-full">
            ...
            <div class="bg-[#E3EFFD] rounded-xl shadow border border-blue-300 p-6 flex flex-col items-center">
                <form @submit.prevent="submit" class="w-full space-y-6">
                    <div>
                        <label for="name" class="block mb-2 text-sm font-bold text-blue-900">Nom du cadeau</label>
                        <input id="name" v-model="form.name" type="text" class="w-full border-gray-300 rounded-xl" />
                    </div>

                    <div>
                        <label for="description" class="block mb-2 text-sm font-bold text-blue-900">Description</label>
                        <textarea id="description" v-model="form.description"
                            class="w-full border-gray-300 rounded-xl"></textarea>
                    </div>

                    <div>
                        <label for="url" class="block mb-2 text-sm font-bold text-blue-900">Lien externe (URL)</label>
                        <input id="url" v-model="form.url" type="url" class="w-full border-gray-300 rounded-xl" />
                    </div>
                    <div v-if="gift.image" class="mb-6 w-full flex flex-col items-start">
                        <p class="text-sm font-semibold text-blue-900 mb-2">
                            Image actuelle :
                        </p>
                        <img :src="'/storage/' + gift.image" alt="Image actuelle du cadeau"
                            class="w-32 h-32 object-cover rounded-xl border border-blue-300" />
                    </div>

                    <div>
                        <label for="image" class="block mb-2 text-sm font-bold text-blue-900">Nouvelle image
                            (facultatif)</label>
                        <div v-if="form.image" class="mt-4 w-full flex flex-col items-start">
                            <p class="text-sm font-semibold text-blue-900 mb-2">
                                Nouvelle image sélectionnée :
                            </p>
                            <img :src="previewImage" alt="Nouvelle image"
