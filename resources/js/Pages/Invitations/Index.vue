...
function deleteInvitation() {
    if (invitationToDelete.value) {
        router.delete(route('invitations.destroy', { invitation: invitationToDelete.value.id }))
        ...
        <div class="not-prose max-w-7xl w-full">
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex space-x-4">
                    ...
                    <button @click="router.visit(route('invitations.edit', event.id))"
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        <tr v-for="invitation in invitations" :key="invitation.id" class="border-t border-green-500">
                            <td class="px-4 py-2 font-bold">{{ invitation.email }}</td>
                            <td class="px-4 py-2">
                                <span v-if="invitation.participant">✅ Oui </span>
                                <span v-else class="italic text-gray-600">En attente</span>
                            </td>
                            <td class="px-4 py-1">
                                <!-- Bouton Supprimer -->
