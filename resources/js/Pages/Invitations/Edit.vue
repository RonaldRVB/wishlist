...
function submit() {
    form.post(route('invitations.storeMultiple'), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset('emails') // ou form.reset() si tu veux tout reset
            form.emails = ['']   // On garde un champ vide par défaut
            router.visit(route('invitations.index', props.eventId))
        },
        onError: () => {
            // facultatif : tu peux ajouter un toast ou focus sur le champ si erreur
        }
    })
}
</script>
    ...
    <div class="w-full min-h-screen bg-[#D6E9FC] py-10 px-6 flex flex-col items-center">
        <div class="not-prose max-w-3xl w-full">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-3xl font-bold text-blue-900">Ajouter des invités</h1>
                ...
                <div class="flex space-x-2">
                    ...
                    <button @click="router.visit(route('invitations.index', eventId))"
