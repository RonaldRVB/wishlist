...
<template>
    ...
    <div class="w-full min-h-screen bg-[#D6E9FC] py-10 px-6 flex flex-col items-center">
        <div class="not-prose max-w-7xl w-full">
            ...
            <!-- INFOS EVENEMENT -->
            <div class="bg-[#E3EFFD] rounded-xl shadow border border-blue-300 p-6 flex flex-col items-start mb-10">
                <img :src="'/storage/' +
                    (event.custom_image || event.default_image?.path) +
                    '?v=' +
                    Date.now()
                    " :alt="event.title" class="max-w-full mx-auto max-h-[250px] rounded shadow mb-6" />

                <p class="mb-2">
                    <span class="font-semibold text-blue-900 pl-5">Description :</span>
                ...
                <p class="mb-2">
                    <span class="font-semibold text-blue-900 pl-5">Visibilité :</span>
            ...
            <div class="bg-[#E3EFFD] shadow rounded-xl overflow-hidden border border-blue-300 w-full">
                ...
                <div v-if="event.is_collaborative"
                    ...
                    <div class="mt-10 w-full mb-16 flex flex-col items-center text-center">
                        <div class="bg-[#E3EFFD] border border-blue-300 rounded-xl shadow px-6 py-4 max-w-xl w-full">
                            <p class="text-gray-800 font-medium mb-4">
                                <span class="italic">Effectue un tirage pour répartir les
                    ...
                    <!-- Résultat du tirage -->
                    <div v-if="drawResult" class="mt-10 w-full bg-white rounded-xl shadow p-6 border border-green-400">
                        <h2 class="text-xl font-bold text-green-800 mb-4">
                            Résultat du tirage
                            <span v-if="drawType === 'invitations'">(via invitations)</span>
                            <span v-else>(via participants)</span>
                        </h2>

                        <ul class="space-y-2">
                            <li v-for="(pair, index) in drawResult" :key="index" class="text-gray-800">
